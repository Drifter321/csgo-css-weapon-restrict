...
public Action:RestrictAdminCmd(client, args)
{
	...
	if(args == 3)
	{
		...
		{
			ReplyToCommand(client, "\x04[SM] %T", "InvalidTeam", client);
			return Plugin_Handled;
		}
	}
...
public Action:UnrestrictAdminCmd(client, args)
{
	if(args < 1)
	{
		ReplyToCommand(client, "\x04[SM] %T", "UnrestrictReply", client);
		return Plugin_Handled;
	}
	decl String:weapon[100];
	GetCmdArg(1, weapon, sizeof(weapon));
...
public Action:KnifeRound(client, args)
{
	...
	{
		ReplyToCommand(client, "\x04[SM] %T", "SpecialRoundAlreadySet", client);
		return Plugin_Handled;
	}
