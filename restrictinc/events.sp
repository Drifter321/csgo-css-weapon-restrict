...
public Action:OnWeaponCanUse(client, weapon)
{
	...
	if(!g_bSpamProtectPrint[client])
	{
		...
		{
			PrintToChat(client, "\x01 \x04[SM] %T %T", weaponNames[_:id], client, "SpecialNotAllowed", client);
		}
		else if(team == CS_TEAM_CT)
		{
			PrintToChat(client, "\x01 \x04[SM] %T %T", weaponNames[_:id], client, "IsRestrictedPickupCT", client, Restrict_GetRestrictValue(team, id));
		}
		else
		{
			PrintToChat(client, "\x01 \x04[SM] %T %T", weaponNames[_:id], client, "IsRestrictedPickupT", client, Restrict_GetRestrictValue(team, id));
		}
...
public Action:CS_OnBuyCommand(client, const String:weapon[])
{
	...
	if(Restrict_IsWarmupRound())
	{
		if(!g_bSpamProtectPrint[client])
		{
			PrintToChat(client, "\x01 \x04[SM] %T", "CannotBuyWarmup", client);
			g_bSpamProtectPrint[client] = true;
			CreateTimer(PRINTDELAY, ResetPrintDelay, client);
		}
	...
	if(result == CanBuy_Block || result == CanBuy_BlockDontDisplay)
	{
		if(team == CS_TEAM_CT && result != CanBuy_BlockDontDisplay)
		{
			if(Restrict_IsSpecialRound() && !Restrict_AllowedForSpecialRound(id))
				...
				PrintToChat(client, "\x01 \x04[SM] %T %T", weaponNames[_:id], client, "IsRestrictedBuyCT", client, Restrict_GetRestrictValue(team, id));
		}
		else if(team == CS_TEAM_T && result != CanBuy_BlockDontDisplay)
		{	if(Restrict_IsSpecialRound() && !Restrict_AllowedForSpecialRound(id))
				PrintToChat(client, "\x01 \x04[SM] %T %T", weaponNames[_:id], client, "SpecialNotAllowed", client);
			else
				PrintToChat(client, "\x01 \x04[SM] %T %T", weaponNames[_:id], client, "IsRestrictedBuyT", client, Restrict_GetRestrictValue(team, id));
		}
